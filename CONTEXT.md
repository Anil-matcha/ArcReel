...
一个生成环节发给模型的完整提示词的定义，正文带数据槽位，随版本内置；渲染时填入项目数据后才是实发文本。
...
_Avoid_: 生成入口（那是准入闸门，见 ADR 0073）、场景、调用点、builder。
...
**模版片段（partial）**：
供提示词模版引用的一段措辞，或是跨模版共享的（`partials/shared/`），或是单个模版按源文类型、资产类型等轴拆出的命名变体（`partials/<模版 id>/`），模版按轴值引用；只被一个模版引用的固定措辞不做片段。
